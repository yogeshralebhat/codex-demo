...
export default function LoginPage() {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage(`Welcome back ${data.user.email}. JWT: ${data.token.slice(0, 20)}...`);
        setPassword('');
      }
    } catch (networkError) {
  ...
  return (
    <main style={{ maxWidth: 480, margin: '0 auto', background: '#fff', padding: '1.5rem', borderRadius: 8 }}>
      ...
      {message ? <p style={{ color: '#166534', overflowWrap: 'anywhere' }}>{message}</p> : null}

      <p>
        Need an account? <Link href="/signup">Go to Sign Up</Link>
      </p>
    </main>
  );
}
